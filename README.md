...
| An OpenRouter API key | Free tier works — see step 3 below if you don't have one yet |
...
| `LLM_API_KEY` | Your OpenRouter API key (`sk-or-...`) — see step 3 |
...
Free-tier models (e.g. `openai/gpt-4o-mini`) work out of the box. Users can also bring their own key (BYOK) via the app itself — see step 2 of the walkthrough below — encrypted at rest with pgcrypto, never stored in plaintext.
...
Once both servers are running, open `http://localhost:5173` and follow these five steps in order:
...
Click the **`LLM: nemotron-3-super (free)`** button in the nav bar to open the BYOK modal. Paste your OpenRouter key, and optionally set a custom model ID/endpoint. It's encrypted at rest via PostgreSQL's `pgcrypto` (`pgp_sym_encrypt`) — never stored in plaintext. Leave every field blank to keep using the server default.
...
User marks a step done or rejected in the UI.
routes_feedback.py scores the step (+1 / -1) and calls cognee.improve() for that user's session.
On the next roadmap fetch, recalled items are matched against current steps using exact significant-title-word overlap (stop-words filtered; ≥2 matching words, or 1 if the title is single-word) — this exactness matters, because loose keyword matching will make every step look "memified" and the reorder will look like it did nothing.
Matched-positive steps get is_memified=True and a positive recall_weight; matched-negative (rejected) steps get is_memified=False and a negative weight, which demotes them to the bottom; unmatched steps get recall_weight=0.0 and sit at neutral position.
Steps are re-sorted by weight and persisted back to Postgres with updated order_index values — the reorder is not just a UI animation, it's a real, durable change to step order.
